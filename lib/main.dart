...
// Импорт провайдера для управления API ключами
import 'providers/api_key_provider.dart';
// Импорт главного экрана приложения
...
void main() async {
  try {
    ...
    // Проверка наличия API ключа (безопасно)
    debugPrint('API Key: ${dotenv.env['OPENROUTER_API_KEY'] != null ? 'configured' : 'not configured'}');
  ...
  Widget build(BuildContext context) {
    ...
    return MultiProvider(
      providers: [
        ...
        // Провайдер для управления API ключами
        ChangeNotifierProvider(
          create: (_) {
            try {
              ...
              debugPrint('Error creating ApiKeyProvider: $e');
              debugPrint('Stack trace: $stackTrace');
              rethrow;
            }
          },
