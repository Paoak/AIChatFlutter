...
class ChatProvider with ChangeNotifier {
  ...
  Future<String?> get baseUrl async => await _api.getCurrentBaseUrl();
  ...
  Future<void> _initializeProvider() async {
    try {
      ...
      // Загрузка доступных моделей (с fallback)
      try {
        await _loadModels();
        _log('Models loaded: $_availableModels');
      } catch (e) {
        _log('Error loading models, using defaults: $e');
      ...
      // Загрузка баланса (с fallback)
      try {
        await _loadBalance();
        _log('Balance loaded: $_balance');
      } catch (e) {
        _log('Error loading balance, using default: $e');
        _balance = 'Balance unavailable';
        notifyListeners();
      }
      
      // Загрузка истории сообщений (с fallback)
      try {
        await _loadHistory();
        _log('History loaded: ${_messages.length} messages');
      } catch (e) {
        _log('Error loading history, using empty: $e');
        _messages.clear();
        notifyListeners();
      }
      
      _log('Provider initialization completed');
    } catch (e, stackTrace) {
      // Логирование критических ошибок инициализации
      _log('Critical error initializing provider: $e');
      _log('Stack trace: $stackTrace');
      
      // Устанавливаем значения по умолчанию для критических ошибок
      _availableModels = [
        {'id': 'deepseek/deepseek-chat-v3-0324:free', 'name': 'DeepSeek V3 0324 (free)'},
        {'id': 'claude-3-sonnet', 'name': 'Claude 3.5 Sonnet'},
        {'id': 'gpt-3.5-turbo', 'name': 'GPT-3.5 Turbo'},
  ...
  Future<void> _loadModels() async {
    try {
      ...
      // Проверяем, что получили валидные модели
      if (_availableModels.isNotEmpty) {
        ...
        // Если API не вернул модели, используем значения по умолчанию
        _availableModels = [
          {'id': 'deepseek/deepseek-chat-v3-0324:free', 'name': 'DeepSeek V3 0324 (free)'},
          {'id': 'claude-3-sonnet', 'name': 'Claude 3.5 Sonnet'},
          {'id': 'gpt-3.5-turbo', 'name': 'GPT-3.5 Turbo'},
      ...
      // Устанавливаем модели по умолчанию при ошибке
      _availableModels = [
        {'id': 'deepseek/deepseek-chat-v3-0324:free', 'name': 'DeepSeek V3 0324 (free)'},
        {'id': 'claude-3-sonnet', 'name': 'Claude 3.5 Sonnet'},
        {'id': 'gpt-3.5-turbo', 'name': 'GPT-3.5 Turbo'},
  ...
  Future<void> _loadBalance() async {
    try {
      ...
      // Проверяем, что получили валидный баланс
      if (_balance.isEmpty || _balance == 'Error') {
        _balance = 'Balance unavailable';
      }
  ...
  Future<void> sendMessage(String content, {bool trackAnalytics = true}) async {
    ...
    if (content.trim().isEmpty) {
      _log('Empty message, skipping send');
      return;
    }
    
    // Проверка наличия модели
    if (_currentModel == null) {
      _log('No model selected, using default');
      if (_availableModels.isNotEmpty) {
        _currentModel = _availableModels[0]['id'];
      } else {
        _log('No models available, cannot send message');
        return;
      }
    }
